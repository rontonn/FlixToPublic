...
public class Request {
    ...
    public convenience init(url: WCURL, method: Method, id: RequestID?) {
        let payload = JSONRPC_2_0.Request(method: method, params: nil, id: JSONRPC_2_0.IDType(id))
        self.init(payload: payload, url: url)
    }

    public convenience init<T: Encodable>(url: WCURL, method: Method, params: [T], id: RequestID?) throws {
    ...
    public convenience init<T: Encodable>(url: WCURL, method: Method, namedParams params: T, id: RequestID?) throws {
        let data = try JSONEncoder.encoder().encode(params)
