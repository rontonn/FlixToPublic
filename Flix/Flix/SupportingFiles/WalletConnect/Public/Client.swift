...
public class Client: WalletConnect {
    ...
    public func eth_signTypedData(url: WCURL,
                                  ...
                                  message: String,
                                  completion: @escaping RequestResponse) throws {
        try sign(url: url, method: method, param1: account, param2: message, completion: completion)
    }
    ...
    private func sign(url: WCURL,
                      ...
                      param2: String,
                      completion: @escaping RequestResponse) throws {
        let request = try Request(url: url, method: method, params: [param1, param2], id: UUID().uuidString)
    ...
    private func handleTransaction(url: WCURL,
                                   ...
                                   transaction: Transaction,
                                   completion: @escaping RequestResponse) throws {
        let request = try Request(url: url, method: method, params: [transaction], id: UUID().uuidString)
        try send(request, completion: completion)
    }
    ...
    public func eth_sendRawTransaction(url: WCURL, data: String, completion: @escaping RequestResponse) throws {
        let request = try Request(url: url, method: "eth_sendRawTransaction", params: [data], id: UUID().uuidString)
        try send(request, completion: completion)
    }
    ...
    override func onConnect(to url: WCURL) {
        ...
        if let existingSession = communicator.session(by: url) {
            ...
            let request = try! Request(url: url, method: "wc_sessionRequest", params: [dAppInfo], id: UUID().uuidString)
            let requestID = request.internalID!
            
            print("\n onConnect..., request = \(request)")
            
            responses.add(requestID: requestID) { [unowned self] rsp in
                self.handleHandshakeResponse(rsp)
            }
    ...
    override func onTextReceive(_ text: String, from url: WCURL) {
        print(" onTextReceive: RAW RESPONSE: text = \(text), from url = \(url)")
        if let response = try? communicator.response(from: text, url: url) {
            log(response)
            print("\n !!! -> onTextReceive: response = \(response)\n")
            print("\n  onTextReceive: responses = \(responses)\n")
            
            
            if let completion = responses.find(requestID: response.internalID) {
                print(" RESPONSE FOUND WITH ID: \(response.internalID)")
                completion(response)
                responses.remove(requestID: response.internalID)
            } else {
                print(" NO RESPONSE FOUND WITH ID: \(response.internalID)")
            }
    ...
    private class Responses: CustomStringConvertible {
