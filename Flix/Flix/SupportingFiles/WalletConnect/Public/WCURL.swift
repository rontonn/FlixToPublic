...
public struct WCURL: Hashable, Codable {
    ...
    public var absoluteString: String? {
        var urlAbsolutString: String?
        if let bridge = bridgeURL.absoluteString.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) {
            urlAbsolutString = "wc:\(topic)@\(version)?bridge=\(bridge)&key=\(key)"
        }
        return urlAbsolutString
    }
